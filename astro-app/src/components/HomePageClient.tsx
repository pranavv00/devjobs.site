...
import AdvancedFiltersPanel, { type AdvancedFilterState } from "@/components/AdvancedFiltersPanel";
import JobCard from "@/components/JobCard";
import { Flame, TrendingUp } from "lucide-react";
import { motion } from "framer-motion";

export default function HomePageClient({ initialJobs }: { initialJobs: Job[] }) {
    ...
    <>
      ...
      <div className="container mx-auto px-4 mt-24">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-10 border-b border-white/5 pb-8 gap-6">
          ...
          <a href="/jobs" className="text-purple-400 font-black uppercase tracking-widest text-[10px] hover:text-white transition-colors group flex items-center gap-2">
          ...
          <>
            ...
            {filteredJobs.length > displayLimit && (
              <div className="flex justify-center mt-20">
                <a href="/jobs" id="view-all-jobs-button" className="px-12 py-4 rounded-2xl bg-white/5 border border-white/10 text-white font-black uppercase tracking-[0.2em] text-xs hover:bg-white/10 hover:border-purple-500/30 transition-all active:scale-95 shadow-2xl">
                  View all jobs
                </a>
              </div>
            )}
      ...
      <section className="container mx-auto px-4 max-w-4xl py-12 mt-12 border-t border-white/5">
        ...
        <div className="flex flex-wrap gap-3">
          <a href="/q/us-remote-jobs" className="px-4 py-2 bg-[#111111] hover:bg-white/5 border border-white/5 hover:border-white/20 rounded-lg text-sm text-zinc-400 hover:text-white transition-all">US Remote Jobs</a>
          <a href="/q/uk-remote-jobs" className="px-4 py-2 bg-[#111111] hover:bg-white/5 border border-white/5 hover:border-white/20 rounded-lg text-sm text-zinc-400 hover:text-white transition-all">UK Remote Jobs</a>
          <a href="/q/growth-marketing-remote" className="px-4 py-2 bg-[#111111] hover:bg-white/5 border border-white/5 hover:border-white/20 rounded-lg text-sm text-zinc-400 hover:text-white transition-all">Growth Marketing Remote</a>
          <a href="/q/react-native-remote" className="px-4 py-2 bg-[#111111] hover:bg-white/5 border border-white/5 hover:border-white/20 rounded-lg text-sm text-zinc-400 hover:text-white transition-all">Remote React Native Developer</a>
          <a href="/q/spanish-remote-jobs" className="px-4 py-2 bg-[#111111] hover:bg-white/5 border border-white/5 hover:border-white/20 rounded-lg text-sm text-zinc-400 hover:text-white transition-all">Spanish Remote Jobs</a>
          <a href="/q/technical-lead-remote" className="px-4 py-2 bg-[#111111] hover:bg-white/5 border border-white/5 hover:border-white/20 rounded-lg text-sm text-zinc-400 hover:text-white transition-all">Technical Lead Remote</a>
          <a href="/q/data-analyst-remote" className="px-4 py-2 bg-[#111111] hover:bg-white/5 border border-white/5 hover:border-white/20 rounded-lg text-sm text-zinc-400 hover:text-white transition-all">Data Analyst Jobs Remote</a>
          <a href="/q/frontend-remote-jobs" className="px-4 py-2 bg-[#111111] hover:bg-white/5 border border-white/5 hover:border-white/20 rounded-lg text-sm text-zinc-400 hover:text-white transition-all">Frontend Remote Jobs</a>
        </div>
      </section>
    </>
  );
